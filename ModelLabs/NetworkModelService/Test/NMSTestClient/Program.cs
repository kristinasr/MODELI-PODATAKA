...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            List<long> lista = tgda.GetExtentValues(ModelCode.OUTAGESCHEDULE);
                            long najveci = lista.Max();
                            Association association = new Association
                            {
                                PropertyId = ModelCode.IRREGULARINTERVALSCHEDULE_IRREGULARTIMEPOINTS,
                                Type = ModelCode.IRREGULARTIMEPOINT
                            };
                            tgda.GetRelatedValues(najveci, association);
                        ...
                        {
                            message = string.Format("GetExtentValues PT failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Get IrregularTimePoint with highest [OUTAGESCHEDULE] GID"); 
